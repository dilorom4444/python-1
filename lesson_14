...
WHERE CHARINDEX(value, fruit_list) > 0
...
SELECT Id, VALS,
       TRY_CAST(LEFT(VALS, PATINDEX('%[^0-9]%', VALS + 'a') - 1) AS INT) AS IntegerAtStart
FROM GetIntegers;
--difficult
SELECT Id,
       STUFF(Vals, 1, 2, SUBSTRING(Vals, 2, 1) + SUBSTRING(Vals, 1, 1)) AS Swapped
FROM MultipleVals;

DECLARE @str VARCHAR(100) = 'sdgfhsdgfhs@121313131';

SELECT SUBSTRING(@str, Number, 1) AS CharValue
...
SELECT 
    ...
    (SELECT STRING_AGG(value, '') 
     FROM STRING_SPLIT(@str, '') 
     WHERE value LIKE '[A-Za-z]') AS Letters;
